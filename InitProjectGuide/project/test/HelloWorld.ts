...
describe("HelloWorld", function () {
  ...
  it("Should not allow anyone to transfer ownership to itself", async function () {
    const { helloWorldContract, accounts } = await loadFixture(deployContract);
    await expect(
      helloWorldContract
        .connect(accounts[1])
  ...
  it("Should change text correctly", async function () {
    const { helloWorldContract, accounts } = await loadFixture(deployContract);
    const tx = await helloWorldContract.setText("Hello World 2");
    await tx.wait();
    const helloWorldText = await helloWorldContract.helloWorld();
    expect(helloWorldText).to.equal("Hello World 2");
  });
});